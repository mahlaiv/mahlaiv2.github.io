...
function buttons(){
    open_button.addEventListener("click", () => {
        ...
        text_field.innerHTML = "Нажата кнопка Open";
        events.push("Нажата кнопка Open");
        localStorage.setItem(new Date(), "Нажата кнопка Open");
...
function draw() {
  ...
  if(Math.abs(firstInitialY - secondInitialY) < orange.clientHeight - 20 + 1 && Math.abs(firstInitialX - secondInitialX) < orange.clientWidth - 20 + 1){
        clearInterval(interval);
        isActive = false;
        randpos();
